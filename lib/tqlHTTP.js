...
var ISE = /^HTTP\/1\.1 500 Internal Server Error: /;
...
function httpRequest(options) {
    ...
    var req = http.request(options, function(res) {
        ...
        if (res.statusCode !== 200) {
            ...
            deferred.reject(new TqlError("http", JSON.stringify(text)));
            return;
        }
        ...
        res.on('end', function() {
            ...
            // the server sends 500s back as the body and doesn't generate an HTTP
            // error
            if (responseBody.search(ISE) !== -1) {
                deferred.reject(new TqlError("http", responseBody));
            } else {
                x2js(responseBody, x2jsOpts, function(e, js) {
                    if (e) {
                        deferred.reject(new TqlError("xml", e.message));
                    } else {
                        deferred.resolve(js);                    
                    }
                });
    ...
    req.on('error', function(e) {
        deferred.reject(new TqlError("http", e.message));
