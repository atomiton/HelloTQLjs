...
var TqlError = require('./tqlUtil').TqlError;
...
Connection.prototype.request = function(transaction) {
    ...
    //
    this._http.post(body.success, {}).then(function(obj){
        // success
        // see if something is wrong with the response
        if (!obj.find || !obj.find.status || obj.find.status != "Success") {
            deferred.reject(new TqlError("tql", JSON.stringify(obj)));
            return;
        }
        ...
        // failure, pass the error object on
        deferred.reject(obj);
    });
